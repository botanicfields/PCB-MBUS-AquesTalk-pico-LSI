...
void AdcWave()
{
  ...
  for (int i = 0; i < fir_out_size; ++i)
    spr1.drawPixel(i, lcd_y_size / 2 - fir_out[i] + ave, TFT_WHITE);
  spr1.pushSprite(160, 100);

  // ellipse on LCD
  spr2.fillSprite(TFT_BLACK);
  spr2.fillEllipse(30, 50,  5, 5 - pp / 20, TFT_CYAN);
  spr2.fillEllipse(90, 50,  5, 5 - pp / 30, TFT_CYAN);
  spr2.drawEllipse(60, 90, 30, 1 + pp /  4, TFT_CYAN);
